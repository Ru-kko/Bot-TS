...
import type { session } from "@bot/types";
...
class SessionsDataBase extends connection {
  ...
  async get(sid: string): Promise<SessionData | undefined> {
    const [data, _] = await this.cnt.query<SessionResponse[]>(`SELECT * FROM sessions_storage WHERE ssid = '${sid}'`);
    const info = data[0];
    if (!info) return;
  ...
  async delete(sid: string): Promise<void> {
    await this.cnt.query(`DELETE FROM sessions_storage WHERE ssid = '${sid}'`);
  }
  ...
  async touch(sid: string, session: SessionData) {
    ...
    await this.cnt.query(`UPDATE sessions_storage SET ${data.join()} WHERE ssid = '${sid}'`);
  }
}

interface SessionResponse extends RowDataPacket, session {}

export default SessionsDataBase ;
