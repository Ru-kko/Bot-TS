...
export  async function channelDelete(channel: Channel){
  ...
  switch (parseInt(ch_json.id)) {
    ...
    case server.wlecome_channel:
      await serverManager.setColunm(["wlecome_channel", 0], ch_json.guildId);
      break;
  ...
  if (server.log_channel !== 0) {
    ...
    const _channel = <TextBasedChannel>bot.client.channels.cache.find((ch) => ch.id === String(server.log_channel))!;
