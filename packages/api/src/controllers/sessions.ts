...
import { AxiosError } from "@bot/dc-auth";
import { getAccessToken, identifyUser } from "@bot/dc-auth";
import { SessionsDB } from "@bot/db";
...
export async function registre(req: Request, res: Response) {
  ...
  const data = await getAccessToken(String(code)).catch<AxiosError<any>>((e) => e);
...
export async function logged(req: Request, res: Response) {
  const seessionManager = new SessionsDB();
  try {
    if (await seessionManager.has(req.sid || "")) {
      res.status(200);
      res.send({
        status: true,
        message: "Discord user with id " + req.session.userid + "is logged",
      });
    } else {
      res.status(200);
      res.send({
        status: false,
        message: "There in not loged",
      });
    }
  } catch (e: Error | any) {
    res.status(500);
