...
CREATE Table IF NOT EXISTS users_pl(
    ...
    CONSTRAINT usr_pl_fk FOREIGN KEY(usr_id) REFERENCES users(usr_id)
);
...
CREATE TABLE IF NOT EXISTS usrs_srvs(
    ...
    CONSTRAINT user_fk FOREIGN KEY (usr_id) REFERENCES users(usr_id),
    CONSTRAINT srv_fk FOREIGN KEY (sv_id) REFERENCES servers(sv_id)
);
-- Sessions Storage
...
CREATE TABLE IF NOT EXISTS sessions_storage(
    ...
    CONSTRAINT user_sess_fk FOREIGN KEY (userid) REFERENCES users(usr_id)
);
